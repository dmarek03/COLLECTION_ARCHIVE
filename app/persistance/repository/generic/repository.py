...
class CrudRepository:
    ...
    def _to_str(value: Any) -> str:
        ...
        return f"'{value}'" if isinstance(value, (str, date, datetime, bytes)) else str(value)

    def _column_values_for_insert(self, item:Any) -> str:
        return ', '.join([self._to_str(value) for keys, value in item.__dict__.items() if keys.lower() != 'id_'])
